...
//			$config['max_height'] = 768;
			...
			if (!is_dir(APPPATH.'../media/img/')) {
				mkdir(APPPATH.'../media/img/', 0777, TRUE);
			}
...
//		die();
	...
	function save_modal($title, $image){
		$data = array(
			'designacao'     => $title,
			'foto' => $image
		);
		...
		return $this->db->get_where('media', array('id_media'=>$id))->row_array();
	}

	function delete($id){
		$file = $this->db->get_where('media', array('id_media'=>$id))->row();
		unlink( APPPATH.'../media/'.$file->foto);
		$this->db->where(array('id_media'=>$id));
